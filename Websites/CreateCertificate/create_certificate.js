...
function generateCertificate() {
  ...
  document.getElementById('output').innerHTML = output;  // No escaping here
...
function generateOutputBlock(title, content, filename) {
  return `
      ...
      <button onclick="downloadContent('${escapeHtml(filename)}', \`${content.replace(/`/g, '\\`')}\`)">Download ${escapeHtml(title)}</button>
  `;
}
